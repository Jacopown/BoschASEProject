...
def main():
    ...
    writer.writerow([
        "step", "label", "mode", "angle_deg", "thickness",
    ...
    try:
        ...
        for step_idx, (label, angle_deg, thickness) in enumerate(EASY_PATH, start=1):
            print(f"\n--- Step {step_idx}/{len(EASY_PATH)}: {label} "
                  f"(angle={angle_deg}, thickness={thickness}) ---")

            # Generate clean image with random bg color
            ...
            # Save images to disk
            clean_name = f"step{step_idx:02d}_{label}_clean.png"
            cv2.imwrite(os.path.join(img_dir, clean_name), clean_img)
            if args.mode == "dirty":
                dirty_name = f"step{step_idx:02d}_{label}_dirty.png"
            ...
            print(f"Predicted: steer={steer:+.4f}, speed={speed:+.4f}")
            ...
            # Write CSV row
            writer.writerow([
                step_idx, label, args.mode, angle_deg, thickness,
